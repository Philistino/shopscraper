...
def test_construct_url():
    assert (
        "https://bingo.com/products.json?limit=2&page=1"
        == scraper._construct_url("bingo.com", 2, 1)
    )
...
def test_yield_product_dicts(product_data_single: dict):
    ...
    assert (
        product_data_single["images"][0].keys()
        == returned[0]["images"][0].keys()
    )
    assert (
        product_data_single["variants"][0].keys()
        == returned[0]["variants"][0].keys()
    )
    assert (
        product_data_single["options"][0].keys()
        == returned[0]["options"][0].keys()
    )
